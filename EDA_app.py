...
# ── guard ──────────────────────────────────────────────────────────────────────
    ...
    if page == PAGES[1]:
        ...
        if high_miss_cols:
            ...
            ins(f"{len(strong)} pairs in range — columns going missing together share the same clinical procedure.")
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        with tab_ov:
            ...
            # skew scatter — more informative than bar for many columns
            ...
            pt_colors  = [C2 if abs(v) > 2 else C1 for v in skew_vals]
            pt_sizes   = [10 if abs(v) > 2 else 6 for v in skew_vals]
            ...
            # skewed columns
            skewed_x = [skew_vals[i] for i in range(len(skew_vals)) if abs(skew_vals[i]) > 2]
            skewed_y = [miss_vals[i]  for i in range(len(miss_vals))  if abs(skew_vals[i]) > 2]
            skewed_t = [skew_cols[i]  for i in range(len(skew_cols))  if abs(skew_vals[i]) > 2]
            fig.add_trace(go.Scatter(
                x=skewed_x, y=skewed_y, mode='markers+text', name='|skew| > 2',
                marker=dict(color=C2, size=10, opacity=0.85),
                text=skewed_t, textposition='top center',
            ...
            st.plotly_chart(fig, use_container_width=True)
            ins(f"Red dots = highly skewed (|skew|>2). Y-axis shows missing % — high missing + high skew columns need extra care.")
        ...
        with tab_drill:
            ...
            # violin plot — shows distribution shape + outliers + group comparison better than density alone
            st.subheader("Violin plot — Survived vs Died")
            fig_v = go.Figure()
            fig_v.add_trace(go.Violin(
                y=surv_clip, name='Survived', side='negative',
                line_color=C1, fillcolor='rgba(47,107,200,0.25)',
                meanline_visible=True, box_visible=True, showlegend=True
            ))
            fig_v.add_trace(go.Violin(
                y=died_clip, name='Died', side='positive',
                line_color=C2, fillcolor='rgba(200,75,47,0.25)',
                meanline_visible=True, box_visible=True, showlegend=True
            ))
            fig_v.update_layout(**PT, height=360,
                                yaxis_title=col_sel, xaxis_title='',
                                violingap=0, violinmode='overlay',
                                legend=dict(orientation='h', yanchor='bottom', y=1.02))
            st.plotly_chart(fig_v, use_container_width=True)

            # ── auto-generated column conclusions ─────────────────────────────
            divider()
            st.subheader(f"📋 Column summary — `{col_sel}`")
            ...
            # distribution shape
            if abs(sk) <= 1:
                bullets.append(f"Approximately symmetric (skew={sk:.2f}) — distribution is close to normal.")
            elif sk > 1:
                bullets.append(f"Right-skewed (skew={sk:.2f}) — most patients have low values, with a long tail of high values. Consider log1p transform.")
            else:
                bullets.append(f"Left-skewed (skew={sk:.2f}) — most patients cluster at high values (e.g. SpO2 near 100% is expected).")

            # separation from target
            if diff_pct_val > 20:
                bullets.append(f"Strong signal: died patients have {diff_direction} mean ({died_m:.3g} vs {surv_m:.3g}, {diff_pct_val:.1f}% difference). Likely a useful feature.")
            elif diff_pct_val > 5:
                bullets.append(f"Moderate signal: died patients have {diff_direction} mean ({died_m:.3g} vs {surv_m:.3g}, {diff_pct_val:.1f}% difference).")
            else:
                bullets.append(f"Weak signal: similar means in survived vs died ({surv_m:.3g} vs {died_m:.3g}, {diff_pct_val:.1f}% difference). May not be a strong predictor.")

            # missing
            if miss_pct_col > 50:
                bullets.append(f"High missingness ({miss_pct_col:.1f}%) — consider dropping this column.")
            elif miss_pct_col > 5:
                bullets.append(f"Moderate missingness ({miss_pct_col:.1f}%) — impute with median before modelling.")
            else:
                bullets.append(f"Low missingness ({miss_pct_col:.1f}%) — safe to use as-is.")

            # negative values
            if neg_count > 0:
                bullets.append(f"⚠️ {neg_count} negative values found — investigate as potential data quality issue.")
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        with tab_corr:
            ...
            top_n = st.slider("Show top N", 10, len(corr_t), 20, key='corr_n')
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        all_num_feats = [c for c in corr_t[corr_t['type']!='categorical']['feature'].tolist()
                         if c in df.columns]
        top_feats = all_num_feats[:30]
        tab_heat, tab_pairs = st.tabs(["🗺️ Heatmap","📋 Redundant pairs"])

        with tab_heat:
            n_feat   = st.slider("Number of top features (by correlation with target)", 10, len(all_num_feats), 30, key='heat_n')
        ...
        with tab_pairs:
            n_pairs = st.slider("Use top N features for pair analysis", 10, len(all_num_feats), 30, key='pairs_n')
            pair_cols = [c for c in corr_t[corr_t['type']!='categorical'].head(n_pairs)['feature'].tolist()
                         if c in df.columns]
            corr_mat2 = df[pair_cols].corr().abs()
            ...
            thr_min, thr_max = st.slider(
                "Show pairs with |r| between",
                min_value=0.0, max_value=1.0,
                value=(0.8, 1.0), step=0.05, key='pair_thr'
            )
            strong = pairs[
                (pairs['abs_r'] >= thr_min) & (pairs['abs_r'] <= thr_max)
            ].reset_index(drop=True)
            st.caption(f"**{len(strong)} pairs** with |r| in [{thr_min:.2f}, {thr_max:.2f}] — consider keeping only one of each pair.")
