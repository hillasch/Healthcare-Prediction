...
# ══════════════════════════════════════════════════════════════════════════
        ...
        with tab_corr:
            ...
            st.markdown(
                "<p style='color:#6a5a4a; font-size:.87rem; margin-bottom:.8rem;'>"
                "This heatmap shows Pearson correlations between the top numeric and binary features most associated with hospital_death. "
                "Each cell shows the correlation between two features. Strong off-diagonal correlations may indicate redundant information or groups of related clinical measurements."
                "</p>", unsafe_allow_html=True
            )
            st.markdown(
                "<p style='color:#6a5a4a; font-size:.87rem; margin-bottom:.8rem;'>"
            ...
            if heatmap_feats:
                hm_corr = df[heatmap_feats].corr()
                # mask upper triangle
                z_full = hm_corr.values.tolist()
                for i in range(len(z_full)):
                    for j in range(i+1, len(z_full[i])):
                        z_full[i][j] = None
                ...
                for i, row in enumerate(z_full):
                    ...
                    for j, val in enumerate(row):
                        text_row.append(f"{val:.2f}" if val is not None else "")
                    text_annot.append(text_row)

                fig_hm = go.Figure(go.Heatmap(
                    z=z_full,
                    x=hm_corr.columns.tolist(),
                    y=hm_corr.index.tolist(),
                ...
                fig_hm.update_layout(**PT,
                                     height=max(500, len(heatmap_feats)*28),
                                     xaxis_tickangle=-50,
                                     xaxis_title='', yaxis_title='',
                                     yaxis_autorange='reversed')
                st.plotly_chart(fig_hm, use_container_width=True)
                st.caption("Only the lower triangle is shown to avoid duplicate correlations. Red = positive correlation, blue = negative correlation.")
